...
{
    // for (size_t i = 0; i < _unsortedNbrVector.size(); i++)
    //     std::cout << YELLOW << " ** " << _unsortedNbrVector[i] << std::endl;
    ...
    for (size_t i = 0; i < _unsortedNbrVector.size() - 1; i+=2)
    {
        ...
        // if (_unsortedNbrVector.size() % 2)
        //     _myPairedVector.push_back(_unsortedNbrVector.back() - 1);
    }
    // std::cout << RED << " Original Pairs:" << std::endl;
    // for (size_t i = 0; i < _myPairedVector.size(); ++i)
    // {
    //     std::cout << RED << " " << _myPairedVector[i].first << " " << _myPairedVector[i].second << std::endl;
    // }
    ...
    // std::cout << GREEN << " Sorted Pairs:" << std::endl;
    // for (size_t i = 0; i < _myPairedVector.size(); ++i)
    // {
    //     std::cout << GREEN << "first " << _myPairedVector[i].first << " second " << _myPairedVector[i].second << " size " << _myPairedVector.size() << std::endl;
    // }
    ...
    // {
    //     std::cout << YELLOW << _sortedNbrVector[i] << " ";
    // }
    // std::cout << "\n";
    int sortedIndex = 0;
	for (int j = 0; jacobsthalSequence[j] - 1 < pair_number; j++)
	{
		...
		int length = index + sortedIndex;
            // std::cout<< RED << index - 1 << " " << sortedIndex - 1 << " " << length << "\n";
	...
	{
		myPairedVectorIt = _myPairedVector.begin();
        // std::cout<< RED << myPairedVectorIt->first << " " << sortedIndex << " " << (myPairedVectorIt + sortedIndex)->first << "\n";
...
void PmergeMe::mergeInsertList()
{
    ...
    // std::cout << "sorted pairs for list: ";
    // for (std::list<std::pair<int,int> >::iterator it1 = myPairedList.begin(); it1 != myPairedList.end(); ++it1)
    // {
